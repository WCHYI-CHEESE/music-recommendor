...
new_df$rating <- round(new_df$rating,2)
new_df <- arrange(new_df, desc(rating))
...
server<- function(input, output) {
  ...
  datasetInput <- reactive ({
    if(input$gender == "Male"){
      rating_table <- filter(new_df, gender=="Male")
    }
    else if(input$gender == "Female"){
      rating_table <- filter(new_df, gender=="Female")
    }
    else{
      rating_table <- filter(new_df, gender=="Mixed")
    }
